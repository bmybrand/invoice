...
import SmmBriefForm from '@/components/SmmBriefForm'
import type { BriefFormPrefill } from '@/lib/brief-form-prefill'

export default function SmmBriefRouteShell({ embedInDashboard = false }: { embedInDashboard?: boolean }) {
  ...
  if (isPortalUser) {
    return isEmployee ? (
      <SmmBriefForm backHref="/dashboard/brief-forms" backLabel="Back to Brief Forms" canSubmit={false} />
    ) : (
      <SmmBriefForm
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
      <div className="mx-auto w-full max-w-[920px] border border-slate-200 bg-white">
        <SmmBriefForm publicView prefill={prefill} />
      </div>
    </div>
  )
}
