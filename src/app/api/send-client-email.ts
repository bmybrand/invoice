...
export async function POST(req: NextRequest) {
  try {
    ...
    if (status === 'approved' || status === 'accepted') {
      ...
      if (password) steps.push(`Use this temporary password for your first login: ${String(password)}`)
      ...
      html = buildEmailTemplate({
        ...
        credentials: [
          ...
          ...(password ? [{ label: 'Temporary password', value: String(password) }] : []),
        ],
      ...
      html = buildEmailTemplate({
        ...
        intro: `Your Invoice CRM registration status is currently: <strong>${escapeHtml(String(status))}</strong>.`,
