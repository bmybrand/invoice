...
type CareerApplicationsAuthResult =
...
export async function requireCareerApplicationsAccess(
  request: Request,
): Promise<CareerApplicationsAuthResult> {
  ...
  if (employeeError) return { ok: false, status: 500, error: 'Failed to verify applications access' }

  const role = normalizeRole((employee as { role?: string } | null)?.role)
  if (role !== 'hr' && role !== 'superadmin') {
    return { ok: false, status: 403, error: 'Only HR or super admins can view job applications' }
  }
